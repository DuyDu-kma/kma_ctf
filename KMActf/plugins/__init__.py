...
from KMActf.utils.decorators import admins_only as admins_only_wrapper
from KMActf.utils.plugins import override_template as utils_override_template
from KMActf.utils.plugins import (
...
from KMActf.utils.plugins import register_script as utils_register_plugin_script
from KMActf.utils.plugins import register_stylesheet as utils_register_plugin_stylesheet
...
def register_plugin_assets_directory(app, base_path, admins_only=False, endpoint=None):
    ...
    :param app: A KMActf application
...
def register_plugin_asset(app, asset_path, admins_only=False, endpoint=None):
    """
    Registers an file path to be served by KMActf
...
def bypass_csrf_protection(f):
    ...
    :return: Returns a function with the _bypass_csrf attribute set which tells KMActf to not require CSRF protection.
    """
    f._bypass_csrf = True
    return f
...
def init_plugins(app):
    """
    Searches for the load function in modules in the KMActf/plugins folder. This function is called with the current KMActf
    app as a parameter. This allows KMActf plugins to modify KMActf's behavior.
    ...
    if app.config.get("SAFE_MODE", False) is False:
        ...
        for module in modules:
            ...
            if os.path.isdir(module) and module_name not in blacklist:
                module = "." + module_name
                module = importlib.import_module(module, package="KMActf.plugins")
                module.load(app)
