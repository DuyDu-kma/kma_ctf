...
from KMActf.models import db, get_class_by_tablename
from KMActf.utils import get_app_config, set_config
from KMActf.utils.migrations import (
    create_database,
    drop_database,
...
class KMActfSerializer(JSONSerializer):
    """
    Slightly modified datafreeze serializer so that we can properly
    export the KMActf database into a zip file.
    """
...
SERIALIZERS["kmactf"] = KMActfSerializer  # Load the custom serializer
...
def import_ctf(backup, erase=True):
    ...
    # Check if the alembic version is from KMActf 1.x
    if alembic_version in (
        ...
        raise Exception(
            "The version of KMActf that this backup is from is too old to be automatically imported."
        )
