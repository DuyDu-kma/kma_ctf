...
class Config(object):
    ...
    CACHE_TYPE:
        ...
        It is important that you specify some sort of cache as KMActf uses it to store values received from the database. If
        no cache is specified, KMActf will default to a simple per-worker cache. The simple cache cannot be effectively used
    ...
    TRUSTED_PROXIES:
        Defines a set of regular expressions used for finding a user's IP address if the KMActf instance
        ...
        KMActf only uses IP addresses for cursory tracking purposes. It is ill-advised to do anything complicated based
    ...
    REVERSE_PROXY:
        ...
        Alternatively if you specify `true` KMActf will default to the above behavior with all proxy settings set to 1.
