...
const loadHint = id => {
  KMActf.api.get_hint({ hintId: id, preview: true }).then(response => {
...
function handleChallengeOptions(event) {
  ...
  let save_challenge = function() {
    KMActf.fetch("/api/v1/challenges/" + params.challenge_id, {
      method: "PATCH",
      credentials: "same-origin",
      headers: {
        Accept: "application/json",
      ...
      .then(function(data) {
        if (data.success) {
          setTimeout(function() {
            window.location =
              KMActf.config.urlRoot + "/admin/challenges/" + params.challenge_id;
          }, 700);
        }
      });
  };
...
$(() => {
  ...
  $("#challenge-update-container > form").submit(function(e) {
    ...
    KMActf.fetch("/api/v1/challenges/" + CHALLENGE_ID + "/flags", {
      method: "GET",
      credentials: "same-origin",
      headers: {
        Accept: "application/json",
