  ...
  KMActf.fetch("/api/v1/users", {
    ...
    .then(function(response) {
      if (response.success) {
        ...
        window.location = KMActf.config.urlRoot + "/admin/users/" + user_id;
      } else {
...
function updateUser(event) {
  ...
  KMActf.fetch("/api/v1/users/" + USER_ID, {
    method: "PATCH",
    credentials: "same-origin",
    headers: {
      Accept: "application/json",
  ...
  ezQuery({
    ...
    success: function() {
      KMActf.fetch("/api/v1/users/" + USER_ID, {
        ...
        .then(function(response) {
          if (response.success) {
            window.location = KMActf.config.urlRoot + "/admin/users";
          }
        });
    }
  });
...
function emailUser(event) {
  ...
  KMActf.fetch("/api/v1/users/" + USER_ID + "/email", {
    method: "POST",
    credentials: "same-origin",
    headers: {
      Accept: "application/json",
...
function deleteUserSubmission(event) {
  ...
  ezQuery({
    ...
    body: body,
    success: function() {
      KMActf.fetch("/api/v1/submissions/" + submission_id, {
        method: "DELETE",
        credentials: "same-origin",
        headers: {
          Accept: "application/json",
...
function deleteUserAward(event) {
  ...
  ezQuery({
    ...
    body: body,
    success: function() {
      KMActf.fetch("/api/v1/awards/" + award_id, {
        method: "DELETE",
        credentials: "same-origin",
        headers: {
          Accept: "application/json",
...
function correctUserSubmission(event) {
  ...
  ezQuery({
    ...
    body: body,
    success: function() {
      KMActf.fetch("/api/v1/submissions", {
        method: "POST",
        credentials: "same-origin",
        headers: {
          Accept: "application/json",
...
const api_funcs = {
  team: [
    x => KMActf.api.get_team_solves({ teamId: x }),
    x => KMActf.api.get_team_fails({ teamId: x }),
    x => KMActf.api.get_team_awards({ teamId: x })
  ],
  user: [
    x => KMActf.api.get_user_solves({ userId: x }),
    x => KMActf.api.get_user_fails({ userId: x }),
    x => KMActf.api.get_user_awards({ userId: x })
  ]
};
