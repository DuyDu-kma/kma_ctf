...
function updateConfigs(event) {
  ...
  KMActf.api.patch_config_list({}, params).then(response => {
...
function uploadLogo(event) {
  ...
  helpers.files.upload(form, {}, function(response) {
    ...
    KMActf.fetch("/api/v1/configs/ctf_logo", {
      method: "PATCH",
...
function importConfig(event) {
  ...
  form_data.append("nonce", KMActf.config.csrfNonce);
  ...
  $.ajax({
    url: KMActf.config.urlRoot + "/admin/import",
    type: "POST",
    data: form_data,
    processData: false,
    contentType: false,
...
function exportConfig(event) {
  event.preventDefault();
  const href = KMActf.config.urlRoot + "/admin/export";
