...
const graph_configs = {
  "#solves-graph": {
    ...
    fn: () => "KMActf_solves_" + new Date().toISOString().slice(0, 19),
    data: () => KMActf.api.get_challenge_solve_statistics(),
    format: response => {
  ...
  "#keys-pie-graph": {
    ...
    fn: () => "KMActf_submissions_" + new Date().toISOString().slice(0, 19),
    data: () => KMActf.api.get_submission_property_counts({ column: "type" }),
    format: response => {
      const data = response.data;
  ...
  "#categories-pie-graph": {
    ...
    data: () => KMActf.api.get_challenge_property_counts({ column: "category" }),
    fn: () => "KMActf_categories_" + new Date().toISOString().slice(0, 19),
    format: response => {
      const data = response.data;
    ...
    layout: annotations => ({
      ...
      yaxis: {
        title: "Percentage of {0} (%)".format(
          KMActf.config.userMode.charAt(0).toUpperCase() +
            KMActf.config.userMode.slice(1)
        ),
        range: [0, 100]
      },
      annotations: annotations
    }),
    data: () => KMActf.api.get_challenge_solve_percentages(),
    fn: () =>
      "KMActf_challenge_percentages_" + new Date().toISOString().slice(0, 19),
    format: response => {
      const data = response.data;
