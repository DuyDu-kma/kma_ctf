...
function get_page_files() {
  return KMActf.fetch("/api/v1/files?type=page", {
...
function show_files(data) {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    wrapper.append(link);
    wrapper.attr("data-location", KMActf.config.urlRoot + "/files/" + f.location);
  ...
  KMActf.fetch(target, {
    ...
    .then(function(response) {
      if (method === "PATCH" && response.success) {
        ...
        window.location =
          KMActf.config.urlRoot + "/admin/pages/" + response.data.id;
      }
    });
}
...
function preview_page() {
  ...
  $("#page-edit").attr("action", KMActf.config.urlRoot + "/admin/pages/preview");
