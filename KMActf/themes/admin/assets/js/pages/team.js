  ...
  KMActf.fetch("/api/v1/teams", {
    ...
    .then(function(response) {
      if (response.success) {
        ...
        window.location = KMActf.config.urlRoot + "/admin/teams/" + team_id;
      } else {
...
function updateTeam(event) {
  ...
  KMActf.fetch("/api/v1/teams/" + TEAM_ID, {
    method: "PATCH",
    credentials: "same-origin",
    headers: {
      Accept: "application/json",
...
$(() => {
  ...
  $(".delete-member").click(function(e) {
    ...
    ezQuery({
      ...
      success: function() {
        KMActf.fetch("/api/v1/teams/" + TEAM_ID + "/members", {
          method: "DELETE",
          body: JSON.stringify(params)
        })
          .then(function(response) {
    ...
    ezQuery({
      ...
      success: function() {
        KMActf.fetch("/api/v1/teams/" + TEAM_ID, {
          ...
          .then(function(response) {
            if (response.success) {
              window.location = KMActf.config.urlRoot + "/admin/teams";
            }
          });
      }
    });
