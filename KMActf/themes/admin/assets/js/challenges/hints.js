...
function hint(id) {
  return KMActf.fetch("/api/v1/hints/" + id + "?preview=true", {
    method: "GET",
...
export function showHintModal(event) {
  ...
  $("#new-hint-edit").on("shown.bs.tab", function(event) {
    if (event.target.hash == "#hint-preview") {
      const renderer = KMActf.lib.markdown();
...
export function showEditHintModal(event) {
  ...
  KMActf.fetch("/api/v1/hints/" + hint_id + "?preview=true", {
    method: "GET",
    credentials: "same-origin",
    headers: {
      Accept: "application/json",
...
export function deleteHint(event) {
  ...
  ezQuery({
    ...
    success: function() {
      KMActf.fetch("/api/v1/hints/" + hint_id, {
        method: "DELETE"
      })
        .then(function(response) {
          return response.json();
