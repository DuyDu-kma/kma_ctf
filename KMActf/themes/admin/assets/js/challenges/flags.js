...
export function deleteFlag(event) {
  ...
  ezQuery({
    ...
    success: function() {
      KMActf.fetch("/api/v1/flags/" + flag_id, {
        method: "DELETE"
      })
...
export function addFlagModal(event) {
  $.get(KMActf.config.urlRoot + "/api/v1/flags/types", function(response) {
...
export function editFlagModal(event) {
  ...
  $.get(KMActf.config.urlRoot + "/api/v1/flags/" + flag_id, function(response) {
    const data = response.data;
    $.get(KMActf.config.urlRoot + data.templates.update, function(template_data) {
...
export function flagTypeSelect(event) {
  ...
  $.get(KMActf.config.urlRoot + "/api/v1/flags/types/" + flag_type_name, function(
    response
  ) {
    const data = response.data;
    $.get(KMActf.config.urlRoot + data.templates.create, function(template_data) {
      const template = nunjucks.compile(template_data);
