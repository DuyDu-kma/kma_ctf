...
const api_func = {
  teams: x => KMActf.api.get_team_solves({ teamId: x }),
  users: x => KMActf.api.get_user_solves({ userId: x })
};

const md = KMActf.lib.markdown();

KMActf._internal.challenge = {};
let challenges = [];
let solves = [];
...
const displayChal = chal => {
  return Promise.all([
    KMActf.api.get_challenge({ challengeId: chal.id }),
    ...
    const challenge = KMActf._internal.challenge;
...
function markSolves() {
  return api_func[KMActf.config.userMode]("me").then(function(response) {
...
function getSolves(id) {
  return KMActf.api.get_challenge_solves({ challengeId: id }).then(response => {
...
function loadChals() {
  return KMActf.api.get_challenge_list().then(function(response) {
    const categories = [];
...
const displayUnlock = id => {
  ezQuery({
    ...
    success: () => {
      ...
      KMActf.api.post_unlock_list({}, params).then(response => {
        if (response.success) {
          KMActf.api.get_hint({ hintId: id }).then(response => {
